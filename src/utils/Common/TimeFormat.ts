...
const createMsFormater = (
	isNormalMode = true,
	order: typeof TimeUnitsOrder = TimeUnitsOrder,
) => {
	...
	function baseFormater(time: number, isChild = false): [string, number] {
...
const createAliases = <
	const R extends TimeRecord,
	M extends Record<keyof R, string[]>,
>(
	base: R,
	modifiers: M,
) => {
...
const sanitizeNumber = (value: string) =>
	Number(value.replaceAll(undescoreAndLastCommaRegex, "").replaceAll(",", "."));
...
const formatToMs = (date: string) => {
	...
	for (const [, value, unit] of matches) {
		...
		if (unitValue === undefined || value === undefined) continue;
