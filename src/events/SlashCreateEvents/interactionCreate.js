const { Interaction, EmbedBuilder, ButtonBuilder, ActionRowBuilder, ButtonStyle } = require("discord.js");
...
module.exports = {
    ...
    async execute(interaction, client) {
        if (devMode && interaction.commandName !== 'devmode' && !developers.includes(interaction.user.id)) {
            const embed = new EmbedBuilder()
                .setDescription(`${client.config.noEmoji} The bot is currently in development mode and commands are disabled.`)
                .setColor(client.config.embedColor);

            const supportButton = new ButtonBuilder()
                .setLabel('Support Server')
                .setStyle(ButtonStyle.Link)
                .setURL('https://discord.gg/pTbFUFdppU')
